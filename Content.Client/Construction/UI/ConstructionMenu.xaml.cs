...
namespace Content.Client.Construction.UI
{
    ...
    public sealed partial class ConstructionMenu : DefaultWindow, IConstructionMenuView, ILocalizedControl
    {
        private string _buildButtonLocKey = "construction-menu-place-ghost";
        private string _favoriteButtonLocKey = "construction-add-favorite-button";
            ...
            string name, string description, Texture iconTexture, bool isItem, bool isFavorite)
        {
            ...
            _buildButtonLocKey = isItem ? "construction-menu-place-ghost" : "construction-menu-craft";
            BuildButton.Text = Loc.GetString(_buildButtonLocKey);
            TargetName.SetMessage(name);
            TargetDesc.SetMessage(description);
            TargetTexture.Texture = iconTexture;
            FavoriteButton.Visible = true;
            _favoriteButtonLocKey = isFavorite ? "construction-add-favorite-button" : "construction-remove-from-favorite-button";
            FavoriteButton.Text = Loc.GetString(_favoriteButtonLocKey);
        }
        ...
        {
            var selectedCategory = OptionCategories.SelectedId;

            Title = Loc.GetString("construction-menu-title");
            SearchBar.PlaceHolder = Loc.GetString("construction-menu-search-placeholder");
            MenuGridViewButton.Text = Loc.GetString("construction-menu-grid-view");
            BuildButton.Text = Loc.GetString(_buildButtonLocKey);
            ...
            {
                OptionCategories.AddItem(Loc.GetString(Categories[i]), i);
            }

            if (selectedCategory >= 0 && selectedCategory < Categories.Length)
                OptionCategories.SelectId(selectedCategory);
        }
    }
}
