...
public abstract partial class SharedVendingMachineSystem
{
    ...
    public bool TryAccessMachinePartial(EntityUid uid, VendingMachineRestockPartialComponent component, VendingMachineComponent machineComponent, EntityUid user, EntityUid target)
    {
        if (!TryComp<WiresPanelComponent>(target, out var panel) || !panel.Open)
        {
            if (_net.IsServer)
    ...
    private void OnAfterInteractPartial(EntityUid uid, VendingMachineRestockPartialComponent component, AfterInteractEvent args)
    {
        if (args.Target is not { } target || !args.CanReach || args.Handled)
            return;
        ...
        if (!TryMatchPackageToMachinePartial(uid, component, machineComponent, args.User, target))
            return;

        if (!TryAccessMachinePartial(uid, component, machineComponent, args.User, target))
            return;
    ...
    public bool TryMatchPackageToMachinePartial(EntityUid uid,
        VendingMachineRestockPartialComponent component,
        VendingMachineComponent machineComponent,
        EntityUid user,
        EntityUid target)
    {
