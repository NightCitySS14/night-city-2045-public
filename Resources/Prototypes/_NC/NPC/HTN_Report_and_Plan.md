...
### Реализованные улучшения NPC HTN

В рамках последнего обновления были добавлены следующие тактические возможности для NPC:

1. **Поиск и использование укрытий**:
   - Добавлен `PickCoverOperator`, который ищет ближайшие статические объекты, блокирующие линию видимости (LOS) для цели.
   - Добавлен `MoveToCoverCompound`, позволяющий NPC перемещаться к выбранному укрытию и удерживать позицию.

2. **Кружение и фланговые манёвры**:
   - Добавлен `CirclingOperator` и новый тип перемещения `JukeType.Circle` в `NPCJukeSystem`.
   - NPC теперь могут двигаться перпендикулярно цели, затрудняя попадание по себе и имитируя обход.

3. **Координация между NPC**:
   - Добавлен `AllyCombatTargetPrecondition`. Если у NPC нет цели, но рядом есть союзник, ведущий бой, NPC «подхватит» цель союзника, обеспечивая групповую атаку.

4. **Логика отступления**:
   - Добавлен `HealthPrecondition`, позволяющий проверять уровень здоровья.
   - Добавлен `FleeOperator`, который заставляет NPC разрывать дистанцию с противником при критическом уровне здоровья.

5. **Разделение поведения по классу оружия**:
   - Введен `NCTacticalCombatCompound`, который применяет разные тактики в зависимости от того, есть ли у NPC огнестрельное оружие.

### Что ещё можно улучшить (будущие задачи)
- Подбор патронов и более сложные операции с боезапасом (перезарядка из инвентаря).
- Использование гранат и спецсредств.
- Более сложные фланговые манёвры с учётом геометрии помещений.
